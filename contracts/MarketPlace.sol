...
uint256 public saleCounter1155;
    ...
    function createSaleOrderFor721(address nftAddress, uint256 tokenId,  uint256 price , address paymentToken) external {
...
function createSaleOrderFor1155( address sftToken, uint256 tokenId, uint256 amount, uint256 price, address paymentToken
) external {
    ...
    require(sft.balanceOf(msg.sender, tokenId) >= amount, "Not enough tokens");

    // Marketplace must be approved
    require(sft.isApprovedForAll(msg.sender, address(this)), "Marketplace not approved");

    // Normalize payment token (address(0) = ETH)
    address normalizePaymentToken = 
        paymentToken == address(0) ? address(0) : paymentToken;
